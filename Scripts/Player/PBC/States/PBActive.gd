...
func enter(msgs: Dictionary = {}) -> void:
	...
	_input_controller.use_skill_index.connect( _on_use_skill_at_index )

func exit() -> void:
	_input_dir = Vector3.ZERO
	_curr_unit.mover.set_input(_input_dir)
	_curr_unit.finished_turn.disconnect( _on_unit_turn_finished )
	_input_controller.use_skill_index.disconnect( _on_use_skill_at_index )
	_curr_unit = null

func physics_update(delta: float) -> void:
	_get_input()
	_curr_unit.mover.set_input(_input_dir, _input_controller.jump_pressed, _input_controller.jump_released)
